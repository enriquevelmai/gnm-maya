...
## Portable Python runtime (downloaded)

On first run, `gnm-maya/runtime/` is populated with the official Windows
**embeddable CPython 3.11.9** (https://www.python.org/downloads/),
redistributed under the **PSF License Agreement** (full text at
https://docs.python.org/3/license.html, also present locally at
`gnm-maya/runtime/LICENSE.txt` once downloaded), so the module runs without a
system Python install.

Its `Lib/site-packages/` contains these dependencies (installed by
